...
{
	public partial class VideoButton 
	{
		public VideoButton()
		{
			InitializeComponent();
		...
		public static readonly DependencyProperty ThumbnailSourceProperty =
			DependencyProperty.Register("ThumbnailSource", typeof(Uri), typeof(VideoButton), 
			new PropertyMetadata(default(Uri), ThumbnailSourcePropertyChangedCallback));
		...
		private static void ThumbnailSourcePropertyChangedCallback(DependencyObject dependencyObject, 
		...
		private void UpdateImage()
		{
			if (ActualWidth > 0.0)
			{
				...
				ImageHelper.SetSource(Image, ThumbnailSource);
			}
		}
