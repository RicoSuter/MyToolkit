...
namespace MyToolkit.WorkflowEngine.Activities
{
    ...
    public abstract class WorkflowActivityBase
    {
        ...
        public virtual async Task<bool> PrepareAsync(WorkflowDataProvider data)
        {
            return false;
        }
        ...
        public virtual async Task<WorkflowActivityResult> CompleteAsync(WorkflowDataProvider data, WorkflowActivityArguments args, CancellationToken cancellationToken)
        {
        ...
        /// <returns>True when the activity should be automatically and immediately executed (with no args). </returns>
        internal virtual Task<bool> PrepareAsync(WorkflowDataProvider data, WorkflowDefinition definition)
        {
            return PrepareAsync(data); 
        }
    }
}