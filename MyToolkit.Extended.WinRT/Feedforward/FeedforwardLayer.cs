...
namespace MyToolkit.MachineLearning.WinRT.Feedforward
{
	public class FeedforwardLayer
	{
		...
		private Matrix _layerMatrix; 
		public Matrix LayerMatrix
        {
            get { return _layerMatrix; }
            set
            {
				_layerMatrix = value;
				if (_layerMatrix != null)
				{
					if (_layerMatrix.Rows < 2)
						throw new Exception("matrix must have at least 2 rows.");
					
					if (_layerMatrix != null && Fire == null)
						Fire = new double[_layerMatrix.Rows - 1];
				}
            }
        }
        ...
        {
            get { return _layerMatrix == null ? 0 : _layerMatrix.Size; }
        }
        ...
        public double[] ComputeOutputs(double[] pattern)
        {
            ...
            for (var i = 0; i < next.NeuronCount; i++)
            {
				...
				next.Fire[i] = LayerActivationFunction.Activate(sum);
            }
