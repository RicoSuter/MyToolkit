...
namespace MyToolkit.Validation
{
		...
		where TClass : ValidatedNotifyPropertyChanged<TClass>
	{
		...
		{
			ClearError(ExpressionUtilities.GetPropertyName(property));
		}
		...
		{
			SetError(ExpressionUtilities.GetPropertyName(property), error);
		}
		...
		{
			return (IEnumerable<ValidationResult>) GetErrors(ExpressionUtilities.GetPropertyName(property));
		}
