...
{
	public class ValidatedNotifyPropertyChanged<TClass> : NotifyPropertyChanged<TClass>, INotifyDataErrorInfo 
		where TClass : ValidatedNotifyPropertyChanged<TClass>
	{
		...
		{
			var result = errors.SingleOrDefault(e => e.Property == propertyName);
			if (result == null)
			{
				if (error == null)
					return;
		...
		{
			return errors.Where(e => e.Error != null).ToList(); 
		}
